...
// AddProduct adds a product to the authenticated user's wishlist.
// Expects a JSON body: { "product_id": 123 }
func (h *ProductWishlistHandler) AddProduct(w http.ResponseWriter, r *http.Request) {
    ...
    if err := h.Repo.Add(r.Context(), uint(userIDUint64), payload.ProductID); err != nil {
...
func (h *ProductWishlistHandler) RemoveProduct(w http.ResponseWriter, r *http.Request) {
    ...
    if err := h.Repo.Remove(r.Context(), uint(userIDUint64), pid); err != nil {
        http.Error(w, err.Error(), http.StatusInternalServerError)
        return
    }
